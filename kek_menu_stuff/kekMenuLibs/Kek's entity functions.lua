...
kek_menu.lib_versions["Kek's entity functions"] = "1.1.6"

local kek_entity = {}

local location_mapper = kek_menu.require("Location mapper")
local vehicle_mapper = kek_menu.require("Vehicle mapper")
local essentials = kek_menu.require("Essentials")
-- Tables
	local table_of_glitch_entity_models <const> = {
		"prop_ld_dstcover_02",
		"prop_torture_ch_01",
		"prop_ld_farm_rail01",
...
-- Get all attached entities
	function kek_entity.get_all_attached_entities(...)
		local Entity <const> = ...
		local entities = {}
		local all_entities <const> = kek_entity.get_table_of_close_entity_type(6)
...
-- Request model
	function kek_entity.request_model(...)
		...
		if math.type(model_hash) == "integer" and streaming.is_model_valid(model_hash) then 
...
-- Max vehicle
	function kek_entity.max_car(...)
		...
		if not only_performance_upgrades then
			local e = {}
			for i = 1, 8 do
				local num <const> = math.ceil(math.maxinteger / math.random(1000000, 2000000000))
			...
			if math.random(1, 20) == 1 then
				local e <const> = vehicle.get_num_vehicle_mods(car, 48)
				if e > 0 then
					vehicle.set_vehicle_mod(car, 48, math.random(1, e))
				end
			end
			for mod_index = 0, 65 do
				local e <const> = vehicle.get_num_vehicle_mods(car, mod_index)
				local num <const> = math.random(0, e)
...
-- Max ped's combat attributes
	function kek_entity.set_combat_attributes(...)
		...
		cover <const>, 
		leave_vehicle <const>, 
		unarmed_fight_armed <const>, 
		taunt_in_vehicle <const>, 
		always_fight <const>, 
		ignore_traffic <const>, 
		use_fireing_weapons <const> = ...
		kek_entity.modify_entity_godmode(Ped, set_godmode)
		for attribute_id, is_on in pairs({
			[0] = cover,
			[1] = use_vehicle,
			[2] = driveby,
			[3] = leave_vehicle,
			[5] = unarmed_fight_armed, 
			[20] = taunt_in_vehicle,
			[46] = always_fight,
			[52] = ignore_traffic,
			[1424] = use_fireing_weapons
		}) do
...
do
	local list_of_end_vehicles <const> = {
		2485144969,
		3505073125,
		4289813342,
		2351681756,
	...
	local vehicle_category_info <const> = {
		{lang["Compacts §"], 0},
		{lang["Sedans §"], 0},
		{lang["SUVs §"], 0},
		{lang["Coupes §"], 0},
	...
	function kek_entity.generate_vehicle_list(...)
		...
		for i2, info in pairs(vehicle_category_info) do
			...
			local parent <const> = menu.add_feature(info[1], "parent", parent.id, function(f)
	...
	function kek_entity.generate_player_vehicle_list(...)
		...
		for i2, info in pairs(vehicle_category_info) do
			...
			local parent <const> = menu.add_player_feature(info[1], "parent", parent, function(f, pid)
				if f.child_count == 0 then
					for i = count - info[2] + 1, count do
						local feature_id <const> = menu.add_player_feature(vehicle_mapper.get_translated_vehicle_name(hashes[i])..initial_name, feature_info[1], f.id, func).id
