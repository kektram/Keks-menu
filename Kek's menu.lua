...
if not utils.dir_exists(paths.kek_menu_stuff.."kekMenuLibs\\Languages\\Vehicle names") then
	essentials.msg(lang["You're missing a file in kekMenuLibs. Please reinstall Kek's menu."], "red", true, 6)
...
for _, properties in pairs({
	...
	{
		setting_name = "Vehicle limit",
		setting = 500
	},
	{
		setting_name = "Ped limit",
		setting = 500
	},
	{
		setting_name = "Object limit",
		setting = 2100
	},
	{
...
for _, properties in pairs({
	...
	},
	{
		...
		setting = true, 
		feature_name = string.format("%s %s", lang["Blacklist"], lang["Notifications"]), 
		feature_parent = u.modder_detection
	},
	{
		setting_name = "Weapon blacklist notifications #notifications#", 
		setting = true, 
		feature_name = string.format("%s %s", lang["Weapon blacklist"], lang["Notifications"]), 
	...
	if f.on then
		for pid in essentials.players() do
			if has_modded_name(pid) then
				essentials.msg(string.format("%s %s", player.get_player_name(pid), lang["has a modded name."]), "orange", true)
				player.set_player_as_modder(pid, keks_custom_modder_flags["Modded-Name"])
			end
		end
		essentials.listeners["player_join"]["modded_name_detection"] = event.add_event_listener("player_join", function(event)
 			if has_modded_name(event.player) then
				essentials.msg(string.format("%s %s", player.get_player_name(event.player), lang["has a modded name."]), "orange", true)
...
do
	...
	settings.toggle["Blacklist"] = menu.add_feature(lang["Blacklist"], "value_str", u.modder_detection_settings.id, function(f)
		if f.on then
...
settings.valuei["Vehicle limit"] = menu.add_feature(lang["Vehicle limit"], "action_value_i", u.debug.id, function(f)
	keys_and_input.input_number_for_feat(f, "")
end)
settings.valuei["Vehicle limit"].min = 10
settings.valuei["Vehicle limit"].max = 3000
settings.valuei["Vehicle limit"].mod = 10

settings.valuei["Ped limit"] = menu.add_feature(lang["Ped limit"], "action_value_i", u.debug.id, function(f)
	keys_and_input.input_number_for_feat(f, "")
end)
settings.valuei["Ped limit"].min = 10
settings.valuei["Ped limit"].max = 2560
settings.valuei["Ped limit"].mod = 10

settings.valuei["Object limit"] = menu.add_feature(lang["Object limit"], "action_value_i", u.debug.id, function(f)
	keys_and_input.input_number_for_feat(f, "")
end)
settings.valuei["Object limit"].min = 10
settings.valuei["Object limit"].max = 23000
settings.valuei["Object limit"].mod = 10
...
settings.toggle["Draw entity limits"] = menu.add_feature(lang["Draw entity limits"], "toggle", u.debug.id, function(f)
	while f.on do
		...
		ui.draw_text(string.format("%s: %i/%i\n%s: %i/%i\n%s: %i/%i", 
			lang["Vehicles"],
			kek_entity.entity_manager.counts.vehicle // 10,
			settings.valuei["Vehicle limit"].value // 10,
			lang["Peds"],
			kek_entity.entity_manager.counts.ped // 10,
			settings.valuei["Ped limit"].value // 10,
			lang["Objects"],
			kek_entity.entity_manager.counts.object // 10,
			settings.valuei["Object limit"].value // 10
			), 
		memoize.v2(0.8, 0.8))		
	end
end)
...
menu.add_feature(lang["Nearby cars have no collision"], "toggle", u.vehicle_friendly.id, function(f)
	...
	while f.on do 
		system.yield(0)
		vehicle_effect_standard(true, func, true)
	end
	vehicle_effect_standard(true, function(car)
			...
			for hash in pairs(weapon_hashes_to_blacklist) do
				...
				for pid in essentials.players() do
					...
					and weapon.has_ped_got_weapon(player.get_player_ped(pid), hash) then
						...
						essentials.msg(string.format("%s %s\'s %s.", lang["Removed"], player.get_player_name(pid), weapon.get_weapon_name(hash)), "orange", settings.in_use["Weapon blacklist notifications #notifications#"])
...
function player_history.add_features(parent, rid, ip, name)
	if parent.child_count == 0 then
		...
		if #what_to_check > 0 then -- In case all toggles are off
			...
			menu.add_feature(lang["Is added to join timeout"], "action_value_str", parent.id):set_str_data({tostring(fake_friend_flags ~= nil and fake_friend_flags & 4 == 4)})
			...
			blacklist_feat = menu.add_feature(string.format("%s: %s", lang["Blacklist reason"], is_blacklisted:match("<(.+)>") or lang["isn't blacklisted"]), "action", parent.id)

			menu.add_feature(string.format("%s: %s", lang["First seen"], seen[1].feat_name), "action", parent.id)
			if #seen > 1 then
				menu.add_feature(string.format("%s: %s", lang["Last seen"], seen[#seen].feat_name), "action", parent.id)
				menu.add_feature(string.format("%s %i %s", lang["Seen"], #seen, lang["times."]), "action", parent.id)
			else
				menu.add_feature(string.format("%s 1 %s", lang["Seen"], lang["time."]), "action", parent.id)
			end
			menu.add_feature(lang["Also known as"]..":", "action_value_str", parent.id):set_str_data({lang["What is known for"]})
			for _, properties in pairs(known_as) do
				if not parent.name:find(properties.name, 1, true) then
					menu.add_feature(string.format("%s [%s]", properties.name, properties.date), "action_value_str", parent.id):set_str_data({properties.matched})
				end
			end
		end
	end
...
do
	...
	settings.toggle["Player history"] = menu.add_feature(lang["Player history"], "toggle", parent.id, function(f)
		while f.on do
			system.yield(0)
			for pid in essentials.players() do
				if not player_history.players_added_to_history[player.get_player_name(pid)] then
...
menu.add_player_feature(lang["Crash"], "action", u.malicious_player_features, function(f, pid)
	local Vehicle <const> = menyoo.spawn_custom_vehicle(paths.kek_menu_stuff.."kekMenuLibs\\data\\Truck.xml", player.player_id())
		...
		if not entity.is_entity_dead(player.get_player_ped(pid)) then
			for Vehicle in essentials.entities(essentials.deep_copy(vehicles)) do
				...
				if kek_entity.get_control_of_entity(Vehicle, 200) then
					...
					kek_entity.teleport(Vehicle, location_mapper.get_most_accurate_position(player.get_player_coords(pid)) + v3(essentials.random_real(-2, 2), essentials.random_real(-2, 2), essentials.random_real(-2, 2)))
				end
			end
		end
	end
...
do
	...
	menu.add_feature(lang["Search"], "action", parent.id, function(f)
		...
		for _, hash in pairs(vehicle.get_all_vehicle_model_hashes()) do
			if vehicle_mapper.get_translated_vehicle_name(hash):find(input) then
		...
		while f.on do
			for pid in essentials.players() do
				if player.is_player_in_any_vehicle(pid) then
					...
					if (not tracker[player_vehicle] or utils.time_ms() > tracker[player_vehicle]) then
						...
						and (not player.is_player_in_any_vehicle(player.player_id()) or player_vehicle ~= player.get_player_vehicle(player.player_id())) then
							...
							local veh_name <const> = vehicle_mapper.get_translated_vehicle_name(p_veh_hash)
...
player_feat_ids["Mad peds"] = menu.add_player_feature(lang["Mad peds in their car"], "action_value_str", u.player_trolling_features, function(f, pid)
	for Vehicle in essentials.entities({player.get_player_vehicle(pid), globals.get_player_global("personal_vehicle", pid)}) do
		if not entity.is_entity_dead(Vehicle) then
			...
			if f.value == 0 and not entity.is_entity_a_ped(vehicle.get_ped_in_vehicle_seat(Vehicle, enums.vehicle_seats.driver) or 0) then
				local Ped <const> = kek_entity.spawn_ped_or_vehicle(hash, function()
...
menu.add_feature(lang["Teleport session"], "value_str", u.session_trolling.id, function(f)
	...
	menu.create_thread(function()
		while f.on do
			entity.set_entity_velocity(essentials.get_most_relevant_entity(player.player_id()), memoize.v3())
			system.yield(0)
		end
	end, nil)
	while f.on do
...
do
	...
	local function block_area(...)
		...
		for i, location in pairs(locations) do
			...
			local object <const> = kek_entity.spawn_object(gameplay.get_hash_key(object_model), function()
	...
	local function unblock_area(...)
		...
		for _, pos in pairs(positions) do
			if player.get_player_coords(player.player_id()):magnitude(pos) > 200 then
	...
	while f.on do
		for pid in essentials.players() do
			if player.get_player_wanted_level(pid) > 0 and player.is_player_playing(pid) and not player.is_player_modder(pid, -1) then
				globals.send_script_event("Remove wanted level", pid, {pid, globals.get_player_global("generic", pid)})
			end
		end
		system.yield(1000)
	end
...
menu.add_feature(lang["Teleport to"], "action_value_str", u.session_trolling.id, function(f)
	for pid in essentials.players() do
		if not player.is_player_modder(pid, -1) then
			if f.value == 0 then
				globals.send_script_event("Send to Perico island", pid, {pid, globals.get_script_event_hash("Send to Perico island"), 0, 0}, true)
			elseif f.value == 1 then
				globals.send_script_event("Send to eclipse", pid, {pid, pid, -1, 0, 128, 1, 1, 20}, true)
			end
		end
	end
end):set_str_data({
			...
			while f.on do
				...
				if settings.toggle["Anti stuck measures"].on then
					...
					if consecutive_stuck_counter > 3 or vehicle.is_vehicle_stuck_on_roof(player.get_player_vehicle(player.player_id())) or (entity.get_entity_submerged_level(player.get_player_vehicle(player.player_id())) == 1 and not streaming.is_model_a_boat(entity.get_entity_model_hash(player.get_player_vehicle(player.player_id())))) then
						consecutive_stuck_counter = 0
						kek_entity.teleport(player.get_player_vehicle(player.player_id()), location_mapper.get_most_accurate_position(player.get_player_coords(player.player_id()) + kek_entity.get_random_offset(-80, 80, 25, 75), true))
					end
				end
...
do
	...
	menu.add_player_feature(lang["Mark as modder"], "action_value_str", u.player_misc_features, function(f, pid)
		if player.is_player_modder(pid, 1 << f.value) then
			...
			player.set_player_as_modder(pid, 1 << f.value)
		end
					...
					for chat_judge_entry in io.lines(paths.chat_judger) do
						...
						if player.is_player_valid(event.player) then
							...
							or (
								...
								elseif f.value == 1 then
									essentials.msg(string.format("%s %s [%s]", lang["Chat judge:\nKicking"], player_name, chat_judge_entry), "orange", settings.in_use["Chat judge #notifications#"])
									globals.send_script_event("Netbail kick", event.player, {event.player, globals.get_player_global("generic", event.player)})
	...
	if f.on then
		essentials.listeners["chat"]["commands"] = event.add_event_listener("chat", function(event)
			if f.data.command_strings[(event.body:match("^%p(%a+)") or ""):lower()] then
				...
				f.data.tracker[player.get_player_scid(event.player)] = utils.time_ms() + 1500
				if player.is_player_modder(event.player, -1) then
					essentials.send_message(string.format("[Chat commands]: You can't use chat commands, %s. You've been marked as a modder.", player.get_player_name(event.player)), event.player == player.player_id())
					return
				end
					...
					if f.on then
						if settings.in_use["Spawn #chat command#"] and str:find("^%pspawn [^\32]+") then
							...
							menu.create_thread(function()
								local Vehicle <const> = kek_entity.spawn_ped_or_vehicle(hash, function() 
									return location_mapper.get_most_accurate_position(kek_entity.get_vector_relative_to_entity(player.get_player_ped(pid), 8)), 0
								end, settings.toggle["Spawn #vehicle# in godmode"].on, settings.toggle["Spawn #vehicle# maxed"].on)
						...
						elseif settings.in_use["Kill #chat command#"] and str:find("^%pkill") and (pid ~= event.player or found_player_pid) then
							if player.is_player_god(pid) then
								essentials.send_message(string.format("[Chat commands] Failed to kill %s; He is in a property or in godmode.", player.get_player_name(pid)), event.player == player.player_id())
							else
								menu.create_thread(function()
									local blame
						...
						elseif settings.in_use["clowns #chat command#"] and str:find("^%pclowns$") then
							menu.create_thread(function()
								...
								if not entity.is_entity_a_vehicle(clown_van) then
									essentials.send_message("[Chat commands]: Failed to spawn clown van; entity limit is reached.", event.player == player.player_id())
								end
							end, nil)
						elseif settings.in_use["chopper #chat command#"] and str:find("^%pchopper$") then
							menu.create_thread(function()
								...
								if not entity.is_entity_a_vehicle(chopper) then
									essentials.send_message("[Chat commands]: Failed to spawn chopper; entity limit is reached.", event.player == player.player_id())
								end
							end, nil)
						elseif settings.in_use["tp #chat command#"] and (str:find("^%ptp [^\32]+") or str:find("^%pteleport [^\32]+")) then
							...
							menu.create_thread(function()
								...
								if player.is_player_valid(essentials.name_to_pid(str:match("^%ptp ([^\32]+)"))) then
									pos = location_mapper.get_most_accurate_position(kek_entity.get_vector_relative_to_entity(player.get_player_ped(essentials.name_to_pid(str:match("^%ptp ([^\32]+)"))), 8))
								end
								if not pos then
						...
						elseif str:find("^%pvotekick$") then
							local scid <const> = player.get_player_scid(pid)
							local name <const> = player.get_player_name(pid)
							local user_scid <const> = player.get_player_scid(event.player)
							local t <const> = f.data.vote_kick_tracker
							...
							t[scid][user_scid] = pid
							local count = 0
							for scid2, pid in pairs(t[scid]) do
...
do
	local chat_command_setting_properties <const> = essentials.const({
		...
		{
			setting_name = "Votekick #chat command#",
			setting = false,
			feature_name = lang["Vote kick"],
			args = "<Player>"
		},
		{
	...
	menu.add_feature(lang["Send command list"], "action_value_str", u.chat_commands.id, function(f)
		settings.toggle["Chat commands"].data.send_chat_commands(f.value == 1)
	end):set_str_data({
		lang["All"],
		lang["Team"]
	})
...
settings.toggle["Display 2take1 notifications"] = menu.add_feature(lang["Display 2take1 notifications on screen"], "toggle", u.display_notifications.id, function(f)
	...
	while f.on do
		if f.data.is_on then
			strings = {}
			file:seek("end", -math.min(file:seek("end"), 1000000))
			for line in file:lines("*l") do
				if not f.data.filter(line, f) then
					strings[#strings + 1] = line
				end
			end
			strings = table.move(strings, #strings - settings.valuei["Number of notifications to display"].max + 1, #strings, 1, {})
			f.data.is_on = false
		end
...
menu.add_feature(lang["Force field"], "value_str", u.force_field.id, function(f)
	if f.on then
		...
		while f.on do
			...
			pos = player.get_player_coords(player.player_id()) + memoize.v3(u.force_field_offset_x.value, u.force_field_offset_y.value, u.force_field_offset_z.value)
...
local function create_custom_vehicle_feature(...)
	...
	local feat = menu.add_feature(essentials.get_safe_feat_name(name), "action_value_str", u.saved_custom_vehicles.id, function(f)
		if f.value == 0 then
			...
			local Vehicle <const> = menyoo.spawn_custom_vehicle(paths.home.."scripts\\Menyoo Vehicles\\"..f.name..".xml", player.player_id())
			kek_entity.vehicle_preferences(Vehicle, true)
		...
		elseif f.value == 2 then
			...
			essentials.rename_file(paths.home.."scripts\\Menyoo Vehicles\\", f.name, input, "xml")
			f.name = input
		end
	end)
	feat.data = "MENYOO"
...
do
	local main_feat <const> = menu.add_feature(lang["Menyoo vehicles"], "action_value_str", u.saved_custom_vehicles.id, function(f)
		...
		elseif f.value == 1 then
			...
			menyoo_saver.save_vehicle(player.get_player_vehicle(player.player_id()), paths.home.."scripts\\Menyoo Vehicles\\"..input..".xml")
			create_custom_vehicle_feature(input)
		elseif f.value == 2 then
			local feats <const> = {}
			for _, feat in pairs(u.saved_custom_vehicles.children) do
				if feat.data == "MENYOO" and not utils.file_exists(paths.home.."scripts\\Menyoo Vehicles\\"..feat.name..".xml") then
					essentials.delete_feature(feat.id)
		...
		local feat <const> = menu.add_feature(essentials.get_safe_feat_name(name), "action_value_str", race_ghost_parent.id, function(f)
			if f.value == 0 then
				if utils.file_exists(paths.home.."scripts\\Race ghosts\\"..f.name..".lua") then
					...
					local Vehicle <const> = kek_entity.spawn_ped_or_vehicle(math.tointeger(hash), function()
						return properties[1].pos, 0 
					end, true, true, nil, false, 10, true)
		...
		local feat = menu.add_feature(essentials.get_safe_feat_name(name), "action_value_str", custom_maps_parent.id, function(f)
			...
			elseif f.value == 1 then
				if essentials.get_file_string(paths.home.."scripts\\Menyoo maps\\"..f.name..".xml", "*a"):find("<ReferenceCoords>", 1, true) then
						...
						if line and line:find("<ReferenceCoords>", 1, true) then
							...
							for i = 1, 3 do
								...
								if line:find("</X>", 1, true) then
									x = tonumber(line:match(">(.-)<"))
								elseif line:find("</Y>", 1, true) then
									y = tonumber(line:match(">(.-)<"))
								elseif line:find("</Z>", 1, true) then
									z = tonumber(line:match(">(.-)<"))
								end
							end
					...
					essentials.msg(lang["Found no spawn."], "red", true, 6)
				end
			elseif f.value == 2 then
				if utils.file_exists(paths.home.."scripts\\Menyoo Maps\\"..f.name..".xml") then
					...
					for line in str:gmatch("[^\n]+") do
						if (not is_existing_ref_pos and line_num == 3) or (is_existing_ref_pos and line:find("<ReferenceCoords>", 1, true)) then
							...
							file:write("\9<ReferenceCoords>\n")
							file:write("\9\9<X>"..pos.x.."</X>\n")
							file:write("\9\9<Y>"..pos.y.."</Y>\n")
							file:write("\9\9<Z>"..pos.z.."</Z>\n")
							file:write("\9</ReferenceCoords>\n")
						end
	...
	local main_feat <const> = menu.add_feature(lang["Menyoo maps"], "action_value_str", custom_maps_parent.id, function(f)
		...
		elseif f.value == 2 then
			...
			for _, feat in pairs(custom_maps_parent.children) do
				if feat.data == "MENYOO" and not utils.file_exists(paths.home.."scripts\\Menyoo Maps\\"..feat.name..".xml") then
					essentials.delete_feature(feat.id)
				elseif feat.data == "MENYOO" then
					feats[#feats + 1] = feat
				end
...
settings.user_entity_features.ped.player_feats["Change ped"] = menu.add_player_feature(lang["What ped to spawn"], "action_value_str", u.player_misc_features, function()
...
settings.user_entity_features.vehicle.feats["Change vehicle"] = menu.add_feature(lang["What vehicle to spawn"], "action_value_str", u.gvehicle.id, function()
...
u.vehicle_fly = menu.add_feature(lang["Vehicle fly"], "toggle", u.gvehicle.id, function(f)
	if f.on then
		...
		while f.on do
			system.yield(0)
			if menu.has_thread_finished(thread) and not entity.is_entity_a_vehicle(fly_entity) then
				thread = menu.create_thread(function()
					fly_entity = kek_entity.spawn_ped_or_vehicle(gameplay.get_hash_key("bmx"), function() 
						return player.get_player_coords(player.player_id()), 0 
					end, true, false, nil, false, 0, true)
...
menu.add_player_feature(lang["Teleport to"], "action_value_str", u.script_stuff, function(f, pid)
	if f.value == 0 then
		globals.send_script_event("Send to Perico island", pid, {pid, globals.get_script_event_hash("Send to Perico island"), 0, 0})
	elseif f.value == 1 then
		globals.send_script_event("Send to eclipse", pid, {pid, pid, -1, 0, 128, 1, 1, 20})
	end
end):set_str_data({
	lang["Perico island"],
	lang["Eclipse"]
...
settings.user_entity_features.vehicle.player_feats["Change vehicle player"] = menu.add_player_feature(lang["What vehicle to spawn"], "action_value_str", u.player_vehicle_features, function()
	local input <const>, status <const> = keys_and_input.input_user_entity("vehicle")
	if status == 2 then
		return
	end
...
menu.add_player_feature(lang["Send Menyoo vehicle attacker"], "action", u.player_trolling_features, function(f, pid)
	...
	for _, file_name in pairs(utils.get_all_files_in_directory(paths.home.."scripts\\Menyoo Vehicles", "xml")) do
		if file_name:lower():find(input:lower(), 1, true) then
			local Vehicle <const> = menyoo.spawn_custom_vehicle(paths.home.."scripts\\Menyoo Vehicles\\"..file_name, pid, false)
			...
			kek_entity.teleport(Vehicle, location_mapper.get_most_accurate_position(player.get_player_coords(pid) + kek_entity.get_random_offset(-80, 80, 45, 75), true), 0)
...
menu.add_player_feature(lang["Float"], "value_str", u.player_trolling_features, function(f, pid)
	...
	while f.on do
		...
		if not entity.is_entity_an_object(platform) then
			...
			platform = kek_entity.spawn_object(hash, function()
				pos = player.get_player_coords(pid) - memoize.v3(0, 0, -2.5)
				return pos
			end)
		end
		local player_coords <const> = player.get_player_coords(pid)
		if entity.get_entity_coords(platform).z > player_coords.z + 3 then
			pos.z = player_coords.z - 2.5
		elseif f.value == 0 then
			pos.z = pos.z + 0.05
		elseif f.value == 2 and entity.get_entity_coords(platform).z + 5 > player_coords.z then
			pos.z = pos.z - 0.05
		end
...
settings.user_entity_features.object.feats["Change object"] = menu.add_feature(lang["Type in what object"], "action_value_str", u.weapons_self.id, function()
	...
	while f.on do
		if ped.is_ped_shooting(player.get_player_ped(player.player_id())) then
			...
			if streaming.is_model_an_object(hash) then
				entities[#entities + 1] = kek_entity.spawn_object(hash, function() 
...
settings.user_entity_features.vehicle.feats["Change vehicle gun"] = menu.add_feature(lang["What vehicle vehicle gun"], "action_value_str", u.weapons_self.id, function()
	local input <const>, status <const> = keys_and_input.input_user_entity("vehicle")
	if status == 2 then
		return
	end
	...
	menu.create_thread(function()
		while f.on do
			if settings.toggle["Show red sphere clear entities"].on and f.value ~= 4 and f.value ~= 5 and radius < 10001 then
				graphics.draw_marker(28, player.get_player_coords(player.player_id()), memoize.v3(0, 90, 0), memoize.v3(0, 90, 0), memoize.v3(radius, radius, radius), 255, 0, 0, 85, false, false, 2, false, nil, "MarkerTypeDebugSphere", false)
			end
			system.yield(0)
		end
	end, nil)
				...
				if parent.child_count == 0 then
					...
					menu.add_feature(lang["Follow entity"], "toggle", parent.id, function(f)
						while f.on and parent.on and entity.is_an_entity(parent.data.entity) do
							...
							kek_entity.teleport(essentials.get_most_relevant_entity(player.player_id()), kek_entity.get_vector_relative_to_entity(parent.data.entity, -5) + memoize.v3(0, 0, 5))
							system.yield(0)
						end
						f.on = false
					end)
	...
	u.entity_manager_toggle = menu.add_feature(lang["Entity manager"], "toggle", u.entity_manager.id, function(f)
		...
		while f.on do
			...
			if time_ms() > player_vehicles.timer then
				player_vehicles = {timer = time_ms() + 2000}
				for pid in essentials.players(true) do
			...
			for i = 1, 3 do -- 1 is vehicle, 2 is ped & 3 is object
				...
				for Entity, parent in pairs(parents_in_use[i]) do
					if is_an_entity(Entity) then
						if time_ms() > (timer[Entity] or 0) then
							timer[Entity] = time_ms() + 500
							if player_vehicles[Entity] then
						...
						parent.on = find(parent.name, filters[i]) ~= nil
					else
						parent.on = false
				...
				local entities <const> = entity_getters[i]()
				for i2 = 1, #entities do
					if not parents_in_use[i][entities[i2]] and is_an_entity(entities[i2]) and (i ~= 2 or not is_ped_a_player(entities[i2])) then
						parents_in_use[i][entities[i2]] = free_parents[i][#free_parents[i]]
						free_parents[i][#free_parents[i]] = nil
						local entity_name <const> = get_names[i](get_entity_model_hash(entities[i2]))
						local parent <const> = parents_in_use[i][entities[i2]]
...
local hotkey_setting_properties <const> = {
	...
	},
	{
		...
		func = function()
			local Vehicle = 0
			if globals.get_player_global("personal_vehicle", player.player_id()) ~= 0 and not entity.is_entity_dead(globals.get_player_global("personal_vehicle", player.player_id())) then
